...
export const invoiceService = {
  ...
  /**
   * Export ALL invoices to an Excel file, ignoring pagination so that every
   * invoice (matching the optional filters) is included in the download.
   * @param status - Optional status filter (PAID, UNPAID, OVERDUE)
   * @param includeArchived - Include archived invoices in the export (default: false)
   * @returns Promise<Blob>
   */
  exportInvoices: async (status?: string, includeArchived: boolean = false): Promise<Blob> => {
