...
export class InvoiceController {
    ...
    @ApiOperation({
        ...
        description: 'Exports all invoices matching the current filters (status, archived) to an Excel file',
    })
    @ApiQuery({
        name: 'status',
        required: false,
    ...
    async exportToExcel(
        ...
        const tenantId = request.tenantId!;
        const buffer = await this.invoiceService.exportToExcel(tenantId, status, includeArchived);
