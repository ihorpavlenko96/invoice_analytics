...
export class InvoiceController {
    ...
    @ApiOperation({
        ...
        description: 'Exports ALL invoices to an Excel file regardless of pagination. By default excludes archived invoices unless includeArchived is true.',
    })
    @ApiQuery({
        name: 'includeArchived',
        required: false,
    ...
    @ApiResponse({
        status: HttpStatus.OK,
        description: 'Excel file with all invoices generated successfully',
    })
    @ApiUnauthorizedResponse({ description: 'Unauthorized' })
    @ApiForbiddenResponse({ description: 'Forbidden - requires SUPER_ADMIN role' })
    async exportToExcel(
        ...
        const tenantId = request.tenantId!;
        const buffer = await this.invoiceService.exportToExcel(tenantId, includeArchived);
