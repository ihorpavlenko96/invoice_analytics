...
export class InvoiceController {
    ...
    // NOTE: @Get('export/excel') MUST be declared before @Get(':id') to prevent
    // NestJS from treating the literal string 'export' as a dynamic :id segment.
    @Get('export/excel')
    @Authorize(RoleName.SUPER_ADMIN)
    @ApiOperation({
        summary: 'Export all invoices to Excel',
        description:
            'Exports ALL non-archived invoices for the tenant to an Excel file, ignoring any pagination or status filters.',
    })
    @ApiResponse({
        status: HttpStatus.OK,
